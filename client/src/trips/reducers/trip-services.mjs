...
const selectTripByTripId = async (tripId) => {
  const command =
    apiRoot + TripActionTypes.SELECT_TRIP_BY_TRIP_ID + "/" + tripId;
...
const getTripsByCountryCode = async (countryCode) => {
  const command =
    apiRoot + TripActionTypes.GET_TRIPS_BY_COUNTRY_CODE + "/" + countryCode;
  console.log(command);
  const response = await fetch(command, {
    method: "GET",
  });
...
const deleteTripByTripId = async (tripId) => {
  const command =
    apiRoot + TripActionTypes.DELETE_TRIP_BY_TRIP_ID + "/" + tripId;
  console.log(command);
  const response = await fetch(command, {
    method: "DELETE",
  });
